...
async def _create_or_update_vpn_key(session: AsyncSession, user: User, server: Server, days: int, lang: str) -> tuple[str, datetime]:
    """
    ...
    """
    logger.info(f"Processing VPN key for user {user.telegram_id} on server {server.name} for {days} days.")
    ...
    if existing_subscription:
        ...
        # Calculate new expiration date by adding days to the current expiry, or to now if it's already passed.
        ...
        try:
            # Call the new update_client function. We don't want to change traffic, so we pass None.
            await xui_client.update_client(
                inbound_id=server.inbound_id,
                uuid=existing_subscription.xui_user_uuid,
                new_expiry_time_ms=new_expire_time_ms,
                new_total_gb=None # Pass None to indicate no change to traffic
            )
            ...
            logger.success(f"Successfully extended subscription for user {user.telegram_id} until {new_expire_time.isoformat()}")
            ...
            # Fallback: if update fails, delete the inconsistent DB record and create a new key
            await session.delete(existing_subscription)
            await session.commit()
            # Continue to the "creation" part of the function
            pass # This will fall through to the creation logic below

    # --- CREATE NEW KEY LOGIC (Unchanged from original, but now serves as fallback) ---
    logger.info(f"No existing subscription found for user {user.telegram_id} on server {server.name} (or update failed). Creating a new key.")
    new_uuid = str(uuid.uuid4())
    expire_time = now + timedelta(days=days)
    expire_time_ms = int(expire_time.timestamp() * 1000)

    try:
        # This part is mostly the same as the original 'create' logic
        client_config = ClientConfig(
            id=new_uuid,
            email=str(user.telegram_id),
            expiryTime=expire_time_ms,
            totalGB=constants.TRIAL_TRAFFIC_MB * 1024 * 1024 if days == constants.TRIAL_PERIOD_DAYS else 0,
        ...
        await session.commit()
        logger.info(f"New subscription for user {user.telegram_id} created in DB.")
...
async def callback_get_free_vpn(callback: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    try:
        vless_link, expire_time = await _create_or_update_vpn_key(session, user, trial_server, constants.TRIAL_PERIOD_DAYS, lang)
